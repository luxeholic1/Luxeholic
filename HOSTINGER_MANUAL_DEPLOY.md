...
- **Deployment**: Git auto-deploys to `~/domains/luxeholic.in/nodejs/`
...
- India: `store.luxeholic.in/wp-json/wc/v3`
- Australia: `au.luxeholic.in/wp-json/wc/v3`
- New Zealand: `nz.luxeholic.in/wp-json/wc/v3`
...
```bash
cd /Users/asmitsharma/Downloads/Luxeholic
npm run build
```
...
# Navigate to domain
cd ~/domains/au.luxeholic.in/public_html

# Clean old files
rm -rf *

# Copy from .in (master copy)
cp -r ~/domains/luxeholic.in/public_html/* .

# Repeat for .co.nz
cd ~/domains/nz.luxeholic.in/public_html
rm -rf *
cp -r ~/domains/luxeholic.in/public_html/* .
```
...
3. Copy files from `luxeholic.in/public_html`
4. Paste to `au.luxeholic.in/public_html`
5. Paste to `nz.luxeholic.in/public_html`
...
```bash
# On server
cd ~/domains/luxeholic.in/nodejs
...
curl -I https://luxeholic.in
curl -I https://au.luxeholic.in
curl -I https://nz.luxeholic.in

# Check title
curl -s https://au.luxeholic.in | grep -o "<title>.*</title>"
