...
/**
 * Seed the blog_posts collection with the original static articles.
 * Usage: npx tsx backend/scripts/seed-blog-posts.ts
 */
...
const SEED_POSTS = [
  {
    ...
    content: [
      ...
      'Luxeholic is highlighting compact 65W and 80W GaN wall charger options for shoppers who want faster charging without carrying multiple adapters. Check current availability, regional pricing, and shipping options before checkout.',
    ],
  },
  {
...
async function main() {
  ...
  for (const post of SEED_POSTS) {
    const existing = await db.collection('blog_posts').findOne({ title: post.title });
    if (existing) {
      console.log(`Skipping (already exists): ${post.title}`);
      continue;
    }
    const created = await blogService.createPost(post);
    console.log(`Seeded: ${created.title} -> /blog/${created.slug}`);
  }
