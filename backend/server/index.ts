...
export async function setupServer(config: ServerConfig = {}): Promise<Express> {
  ...
  const siteBaseUrl = (hostHeader?: string) => {
    const host = String(hostHeader || 'luxeholic.in').replace(/^www\./, '').split(':')[0];
    if (host === 'luxeholic.com.au') return 'https://luxeholic.com.au';
    if (host === 'luxeholic.co.nz') return 'https://luxeholic.co.nz';
    return 'https://luxeholic.in';
  };
  ...
  if (clientDistPath) {
    ...
    app.get('/', (_req, res) => {
      res.type('html').send(`
        <html><head><meta charset="utf-8"/><title>Luxeholic API</title></head>
        <body style="font-family:system-ui;padding:32px">
          <h1>Luxeholic backend is running</h1>
...
if (isDirectRun) {
  console.log('🚀 Starting Luxeholic backend...');
  console.log('📋 Env:', {
