...
function buildMetaDescription(wooProduct: any, cleanName: string, categories: Array<{ name: string }>): string {
  ...
  const base = source || `Shop ${cleanName} online at Luxeholic with secure checkout and regional delivery.`;
  const normalized = `${base} ${category ? `Browse more ${category}products at Luxeholic.` : ''}`
    .replace(/\s+/g, ' ')
    .trim();
...
export function createProductDocument(wooProduct: any, variations?: any[]): MongoProduct {
  ...
  return {
    ...
    searchText,
    seo: {
      title: `${cleanName || wooProduct.name} | Luxeholic`.slice(0, 68),
