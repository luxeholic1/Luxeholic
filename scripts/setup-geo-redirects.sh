...
# Geo-redirect setup script for Luxeholic multidomain
# This script helps set up geo-based redirects from main domain to regional domains

echo "🌍 Setting up Geo-Redirects for Luxeholic Multi-Domain"
echo ""
...
export function middleware(request: NextRequest) {
  ...
  if (!hostname.includes('luxeholic.com') || hostname.includes('.com.au') || hostname.includes('.co.nz')) {
  ...
  try {
    ...
    // if (country === 'AU') {
    //   return NextResponse.redirect(new URL(`https://au.luxeholic.in${pathname}`, request.url));
    // }
...
echo ""
echo "1. 🌐 Domain Setup:"
echo "   - Point luxeholic.com → Vercel deployment"
echo "   - Point au.luxeholic.in → Same Vercel deployment"
echo "   - Point nz.luxeholic.in → Same Vercel deployment"
echo ""
...
echo ""
echo "4. 🧪 Test:"
echo "   - Visit luxeholic.com from different locations"
