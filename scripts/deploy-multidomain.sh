...
# Multi-domain deployment script for Luxeholic
...
# Default values
TARGET_DOMAIN=${1:-"luxeholic.com"}
PLATFORM=${2:-"vercel"}  # vercel or hostinger

echo -e "${BLUE}🚀 Deploying Luxeholic to ${TARGET_DOMAIN} on ${PLATFORM}${NC}"
...
deploy_to_vercel() {
    ...
    case $domain in
        "luxeholic.com")
            ...
            ;;
        "au.luxeholic.in")
            npx vercel --prod --name luxeholic-au
            ;;
        "nz.luxeholic.in")
            npx vercel --prod --name luxeholic-nz
            ;;
        "luxeholic.in")
            npx vercel --prod --name luxeholic-in
            ;;
        "luxeholic.in.au")
            npx vercel --prod --name luxeholic-in-au
            ;;
        "luxeholic.in.nz")
            npx vercel --prod --name luxeholic-in-nz
            ;;
        *)
