#!/bin/bash
# Multi-domain build script for Luxeholic
...
case $DOMAIN in
    india)
        ENV_FILE=".env.india"
        SERVER_PATH="server:/var/www/luxeholic.in/"
        ;;
    australia)
        ENV_FILE=".env.australia"
        SERVER_PATH="server:/var/www/au.luxeholic.in/"
        ;;
    newzealand)
        ENV_FILE=".env.newzealand"
        SERVER_PATH="server:/var/www/nz.luxeholic.in/"
        ;;
    *)
