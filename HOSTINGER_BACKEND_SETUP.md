# Hostinger Backend Setup - au.luxeholic.in
...
1. **In Hostinger Panel → Node.js:**
   ...
   - **Application URL:** `https://au.luxeholic.in`
...
```apache
# Luxeholic Hostinger / Apache config
...
# Check from outside
curl https://au.luxeholic.in/api/status
...
# Check for errors
pm2 logs luxeholic-server --err --lines 50
```
...
1. **Check if Node server is running:**
   ```bash
   pm2 list
   # Should show "luxeholic-server" with status "online"
   ```
...
1. **Check logs:**
   ```bash
   pm2 logs luxeholic-server --lines 100
   ```
...
3. **Restart server:**
   ```bash
   pm2 restart luxeholic-server
   # OR
   pm2 delete luxeholic-server
   pm2 start ecosystem.config.cjs
   ```
...
✅ API works externally: `curl https://au.luxeholic.in/api/status`  
✅ Website loads: https://au.luxeholic.in  
✅ No 503 errors  
✅ Products API works: https://au.luxeholic.in/api/products  
