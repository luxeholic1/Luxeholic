...
// luxeholic.com.au and luxeholic.co.nz are static-only Hostinger deployments
// with no Node backend behind them, so a relative /api/* call just hits the
// SPA's index.html instead of the Express server. Route those two domains to
// the main (luxeholic.in) domain's backend instead, tagging the request with
// `region` so the server's WooCommerce-fallback store selection still picks
// AU/NZ pricing — the Host header can't be overridden from a cross-origin
// browser fetch.
const CROSS_DOMAIN_REGION = (() => {
  if (typeof window === 'undefined') return null;
  const host = window.location.hostname.replace(/^www\./, '');
  if (host === 'luxeholic.com.au') return 'AU';
  if (host === 'luxeholic.co.nz') return 'NZ';
  return null;
})();
const MAIN_BACKEND_ORIGIN = 'https://luxeholic.in';

export const apiUrl = (path: string) => {
  const fullPath = path.startsWith('/') ? path : `/${path}`;
  if (!CROSS_DOMAIN_REGION) return `${BACKEND_URL}${fullPath}`;
  const separator = fullPath.includes('?') ? '&' : '?';
  return `${MAIN_BACKEND_ORIGIN}${fullPath}${separator}region=${CROSS_DOMAIN_REGION}`;
};
...
const ALL_STORES = [
  {
    ...
    storeBase: 'https://store.luxeholic.in',
  ...
  },
  {
    ...
    storeBase: 'https://store.luxeholic.com.au',
  ...
  },
  {
    ...
    storeBase: 'https://store.luxeholic.co.nz',
