...
const RECENT_SEARCHES_KEY = "luxeholic_recent_searches";
...
function scoreSuggestion(product: Product, query: string) {
  // Category is deliberately excluded — see Shop.tsx's scoreProduct for why:
  // a query that exactly matches a category name would otherwise give every
  // product in that category an identical, dominant score regardless of
  // whether the product itself is relevant.
...
const Navbar = () => {
  ...
  const [navigableItems, setNavigableItems] = useState<{ href: string }[]>([]);
  ...
  // Arrow-key highlight resets whenever the query (and therefore the
  // suggestion list) changes, so a stale index doesn't point at the wrong item.
  useEffect(() => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    // A highlighted suggestion takes priority over the raw typed query — Enter
    // should go to the item the user arrowed to, same as a real autocomplete.
    if (highlightedIndex >= 0 && navigableItems[highlightedIndex]) {
      navigate(navigableItems[highlightedIndex].href);
  ...
  const runRecentSearch = (q: string) => {
    navigate(`/shop?q=${encodeURIComponent(normalizeSmartQuery(q))}`);
            ...
            >
              <div className="mx-auto max-w-3xl px-4 py-2 sm:px-6">
                <div className="relative">
                  ...
                  {/* Recent searches — shown before the user has typed enough for suggestions */}
                  <AnimatePresence>
                    {searchOpen && searchQuery.trim().length === 0 && recentSearches.length > 0 && (
                      <motion.div
                        initial={{ opacity: 0, y: 8 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 8 }}
                      ...
                      >
                        <div className="flex items-center justify-between px-3 pt-2.5">
                          ...
                          <button
                            ...
                            className="text-[10px] font-semibold text-gray-400 hover:text-primary transition-colors"
                          >
                            Clear
                          </button>
                        </div>
                    ...
                    {searchQuery.trim().length >= 2 && searchOpen && (
                      ...
                      >
                        <SearchSuggestions
                          ...
                          onItemsChange={setNavigableItems}
...
const SearchSuggestions = ({
  ...
  onItemsChange?: (items: { href: string }[]) => void;
  onSelect: () => void;
}) => {
  ...
  // Report the product suggestions (in render order) up to the parent so
  // arrow-key navigation and Enter-to-select know what's currently on screen.
  useEffect(() => {
    onItemsChange?.(rankedSuggestions.map((product) => ({ href: `/product/${product.slug}` })));
  }, [rankedSuggestions, onItemsChange]);
  ...
  return (
    <div className="max-h-[360px] overflow-y-auto py-1.5 scrollbar-hidden">
      ...
      {rankedSuggestions.map((product, index) => (
        <Link
          key={product.id}
          to={`/product/${product.slug}`}
          onClick={onSelect}
          className={cn(
            "flex items-center gap-3 border-b border-gray-100 px-3 py-2.5 transition-colors hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800 last:border-0",
            index === highlightedIndex && "bg-gray-50 ring-1 ring-inset ring-primary/40 dark:bg-gray-800"
          )}
        >
