...
import { Zap, Globe, ChevronDown, ArrowRight, Loader2 } from "lucide-react";
import { countries, CountryInfo, useCurrency } from "@/context/CurrencyContext";
...
const SplashScreen = ({ onEnter }: SplashScreenProps) => {
  const { country, isLoadingLocation } = useCurrency();
  ...
  // Update selected country when geolocation detects a new one
  useEffect(() => {
    if (!isLoadingLocation) {
      setSelectedCountry(country);
    }
  }, [country, isLoadingLocation]);
          ...
          <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-2xl">
            ...
            >
              ...
              <h1 className="font-display font-bold text-4xl sm:text-5xl md:text-7xl tracking-tight leading-[0.95]">
                Enter the world of
                <br />
                <span className="text-gradient">Luxeholic</span>
              </h1>
              <p className="mt-4 sm:mt-6 text-sm sm:text-base text-muted-foreground max-w-md mx-auto leading-relaxed">
                Premium electronics from the world's finest brands. {isLoadingLocation ? 'Detecting your location...' : 'Choose your region to explore curated tech for your market.'}
              </p>
            </motion.div>
            ...
            >
              <div className="relative">
                <button
                  onClick={() => !isLoadingLocation && setDropdownOpen(!dropdownOpen)}
                  disabled={isLoadingLocation}
                  className="w-full flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-md px-5 py-4 text-left hover:border-primary/40 transition-all duration-300 group disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold">
                        {isLoadingLocation ? 'Detecting location...' : selectedCountry.name}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {isLoadingLocation ? 'Please wait...' : `${selectedCountry.currency} · luxeholic${selectedCountry.domain}`}
                      </p>
                    </div>
                  </div>
                  {!isLoadingLocation && (
            ...
            >
              <button
                onClick={handleEnter}
                disabled={isLoadingLocation}
                className="group relative inline-flex items-center gap-3 rounded-full bg-gradient-brand px-10 py-4 sm:px-12 sm:py-5 text-sm sm:text-base font-bold text-white shadow-glow hover:shadow-glow-pink transition-all duration-500 hover:scale-105 overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
              >
                <span className="relative z-10">
                  {isLoadingLocation ? 'Detecting Location...' : 'Enter Store'}
                </span>
                {!isLoadingLocation && (
                  <ArrowRight className="relative z-10 h-4 w-4 sm:h-5 sm:w-5 transition-transform group-hover:translate-x-1" />
                )}
                {isLoadingLocation && (
                  <Loader2 className="relative z-10 h-4 w-4 sm:h-5 sm:w-5 animate-spin" />
                )}
                {!isLoadingLocation && (
                  <motion.div
                    animate={{ x: ["-100%", "200%"] }}
