...
export default function AdminBlogs() {
  ...
  const handleMediaUpload = async (file: File, kind: "image" | "video") => {
    ...
    try {
      ...
      const response = await fetch(apiUrl("/api/blogs/upload"), { method: "POST", body });
      const json = await response.json();
  ...
  const handlePdfImport = async (file: File) => {
    ...
    try {
      const body = new FormData();
      body.append("file", file);
      const response = await fetch(apiUrl("/api/blogs/parse-pdf"), { method: "POST", body });
      const json = await response.json();
  ...
  const handleHtmlImport = async (file: File) => {
    ...
    try {
      const body = new FormData();
      body.append("file", file);
      const response = await fetch(apiUrl("/api/blogs/parse-html"), { method: "POST", body });
      const json = await response.json();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(apiUrl(editingId ? `/api/blogs/${editingId}` : "/api/blogs"), {
  ...
  const handleDelete = async (post: BlogPost) => {
    ...
    try {
      const response = await fetch(apiUrl(`/api/blogs/${post._id}`), { method: "DELETE" });
      const json = await response.json();
